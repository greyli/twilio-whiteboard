(function() {
    ...
    fetch('/token').then(function(response) {
        ...
        syncClient.on('connectionStateChanged', function(state) {
            if (state !== 'connected') {
                message.textContent = 'Sync is not live (websocket connection <span style="color: red">' + state + '</span>)…'
            } else {
                message.textContent = 'Sync is live!'
            }
        })
    ...
    function changeColor() {
        ...
        colorBtn.style.border = '5px solid ' + current.color
    }
